...
import com.yunluo.android.arcadehub.AboutActivity;
import com.yunluo.android.arcadehub.Emulator;
import com.yunluo.android.arcadehub.GamePlayActivity;
import com.yunluo.android.arcadehub.HelpActivity;
import com.yunluo.android.arcadehub.R;
import com.yunluo.android.arcadehub.GameSettingActivity;
import com.yunluo.android.arcadehub.cheat.CheatActivity;
...
import com.yunluo.android.arcadehub.prefs.GameFilterPrefs;
import com.yunluo.android.arcadehub.save.ArchiveSaveActivity;
import com.yunluo.android.arcadehub.save.ArchiveSubActivity;
...
public class MainHelper {
    ...
    final static public int SUBACTIVITY_ABOUT = 3;

    final static public int SUBACTIVITY_RECOMMEND = 4;
    ...
    public MainHelper(GamePlayActivity value) {
        mm = value;
    ...
    public void updateEmuValues() {
        ...
        int sfr = 2048;
        try {
            sfr = Integer.valueOf(am.getProperty(AudioManager.PROPERTY_OUTPUT_FRAMES_PER_BUFFER))
                    .intValue();
        ...
        int sr = 44100;
        try {
            sr = Integer.valueOf(am.getProperty(AudioManager.PROPERTY_OUTPUT_SAMPLE_RATE))
                    .intValue();
        } catch (Error e) {
        }
    ...
    public void updateMAME4droid() {
        ...
        if (this.getscrOrientation() == Configuration.ORIENTATION_PORTRAIT) {
            ...
            if (ControlCustomizer.isEnabled() && !Emulator.isPortraitFull()) {
                ...
                mm.getDialogHelper().setInfoMsg(res.getString(R.string.m_help_setting_hint));
                mm.showDialog(DialogHelper.DIALOG_INFO);
            }
        } else {
        ...
        if (Emulator.isInMAME()
                && Emulator.getValue(Emulator.IN_MENU) == 0
    ...
    public void showCheat() {
		Intent intent = new Intent(mm, CheatActivity.class);
		intent.putExtra("HSAH_CODE", this.hashCode());
     ...
     * save
     */
    public void showArchiveSubView() {
    	String name = SharePreferenceUtil.loadName(mm);
		Emulator.pause();
		
		Bundle bun = new Bundle();
		bun.putString("ROM_NAME", name);
		bun.putBoolean("IN_GAME", true);
		Intent intent = new Intent(mm, ArchiveSubActivity.class);
     ...
     * load
     */
    public void showArchiveSaveView() {
    	String name = SharePreferenceUtil.loadName(mm);
		Emulator.pause();
		
		Bundle bun = new Bundle();
		bun.putString("ROM_NAME", name);
		Intent intent = new Intent(mm, ArchiveSaveActivity.class);
		intent.putExtras(bun);
		mm.startActivity(intent);
    }
    ...
    public void showSettings() {
        Intent i = new Intent(mm, GameSettingActivity.class);
    ...
    public void showAbout() {
    	Intent i3 = new Intent(mm, AboutActivity.class);
    	mm.startActivityForResult(i3, MainHelper.SUBACTIVITY_ABOUT);
    }
    ...
    public void showCombination(){
        Combination aa = new Combination(mm,R.style.myDialogTheme,true);
    }
    ...
    public ArrayList<Integer> measureWindow(int widthMeasureSpec, int heightMeasureSpec,
            int scaleType) {
        ...
        if (scaleType == PrefsHelper.PREF_STRETCH)// FILL ALL
        {
            ...
            if (scaleType == PrefsHelper.PREF_SCALE)
                scale = Math.min((float)widthSize / (float)w, (float)heightSize / (float)h);
